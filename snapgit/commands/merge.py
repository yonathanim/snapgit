...
def merge(branch_name: str) -> None:
    ...
    try:
        ...
        if current_commit is None:
            raise ValidationError("Current branch has no commits yet")
        
        # Validate the target branch exists and is valid
        SafetyValidator.validate_branch_exists(branch_name)
        
        if current_branch == branch_name:
            raise ValidationError("Cannot merge a branch into itself")
        ...
        if not success:
            ...
            raise ValidationError("Merge failed due to conflicts")
